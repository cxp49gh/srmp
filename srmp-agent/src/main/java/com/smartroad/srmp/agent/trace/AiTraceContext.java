...
public class AiTraceContext {
    ...
    public static class StepTimer {
        ...
        public void success() { success(null); }
        public void success(Integer count) { finish("SUCCESS", count, null); }
        public void skipped() { finish("SKIPPED", 0, null); }
        public void failed(Throwable error) { finish("FAILED", null, error == null ? null : error.getMessage()); }
        public void timeout(Throwable error) { finish("TIMEOUT", null, error == null ? null : error.getMessage()); }
        private void finish(String status, Integer count, String error) {
            AiTraceStep step = new AiTraceStep(); step.setName(name); step.setLabel(label); step.setStatus(status);
            step.setCostMs(System.currentTimeMillis() - startMs); step.setCount(count); step.setError(error); context.addStep(step);
        }
    }
}
