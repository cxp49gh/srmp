...
模板中出现的新变量必须先登记，再进入 SRMP 模板库做渲染预览。""", rag=False), "00_知识库治理")
    ...
    add("10_指标与评定标准", category_doc("ASSESSMENT_STANDARD", "维护 MQI、PQI、PCI、RQI 等指标解释、计算口径和等级判定规则。", [
        "MQI 指标解释", "PQI 指标解释", "PCI 指标解释", "RQI 指标解释", "RDI 指标解释", "SCI/BCI/TCI 指标解释", "评定等级与判定规则", "指标异常组合判读卡",
    ]))
    indicator_samples = {
        "MQI 指标解释": """### 标准定位

MQI 是公路技术状况综合指数，是综合技术状况入口。JTG 5210—2018 发布后，MQI 及分项指标用于支撑养护分析、优先级排序和后续复评。

### 直接判读规则

- 先看 MQI 总体等级，再看分项最低项；如果 MQI 为良但 PCI 为中，回答重点应落在路面破损薄弱项，而不是泛泛说“总体尚可”。
- MQI 偏低时，按“路面 PQI、路基 SCI、桥隧 BCI、沿线设施 TCI”逐项定位短板。
- 不得只凭 MQI 直接给出铣刨重铺；必须结合 PCI、RQI、RDI、病害类型、分布范围和现场复核结论。

### AI 可直接引用

“MQI 只能说明综合技术状况，不能直接决定工法。建议先看分项最低项：若 PCI 低，重点关联裂缝、坑槽、松散、沉陷；若 RQI 低，重点复核平整度和行驶舒适性；若 RDI 低，重点复核车辙深度、重载和渠化交通。”""",
        "PQI 指标解释": """### 标准定位

PQI 表示路面使用性能状况，适合用作路面层面的综合判断。它不是单一病害指标，低分时必须拆解到路面破损、平整度、车辙、抗滑或结构强度等可用数据。

### 直接判读规则

- PQI 低且 PCI 低：优先解释为路面破损问题，需关联裂缝、坑槽、松散、沉陷等病害。
- PQI 低但 PCI 尚可：优先检查 RQI/RDI 等分项，判断是否由平整度、车辙或行驶质量问题驱动。
- PQI 低且病害集中在连续区间：建议按连续病害区组织小段集中处置，而不是按零散点位逐个处理。
- PQI 数据缺少分项时：回答应提示“需要补充 PCI/RQI/RDI 或病害明细后再确定处置工法”。

### AI 可直接引用

“PQI 是路面使用性能的综合结果，处置建议应先拆分薄弱分项；当前若 PCI 同时偏低，应优先定位裂缝、坑槽等破损证据。”""",
        "PCI 指标解释": """### 标准定位

PCI 是路面损坏状况指数，用于解释路面破损程度。它应与裂缝、坑槽、松散、沉陷、拥包、车辙等病害明细一起使用。

### 直接判读规则

- PCI 处于中/次/差时，应输出“破损较明显/较严重”的有条件判断，并列出支撑病害。
- PCI 下降但病害记录少：提示核查检测批次、图层同步、桩号范围和现场照片，不能直接判定无病害。
- PCI 低且裂缝密集：优先考虑灌缝、封缝、封层或局部铣刨重铺，具体取决于裂缝宽度、密度、是否渗水和基层稳定性。
- PCI 低且坑槽多：先处理安全风险，再按临时修补/永久修补分层闭环。

### AI 可直接引用

“PCI 反映路面损坏状况；当前 PCI 偏低时，应先列明裂缝、坑槽、松散、沉陷等证据，再给出处置建议。没有病害明细时，只能建议补充现场复核，不能直接下大修结论。”""",
        "RQI 指标解释": """### 标准定位

RQI 表示路面行驶质量，主要反映平整度和行车舒适性。它常与 IRI、跳车、沉陷、拥包、桥头跳车和连续修补不平整有关。

### 直接判读规则

- RQI 低但 PCI 不低：优先怀疑平整度问题、沉陷/拥包、桥涵过渡段跳车或修补后不平整。
- RQI 低且用户反馈颠簸：建议补充平整度检测或现场复核，不应只按裂缝数量解释。
- RQI 低且局部沉陷：需判断是否存在基层松散、脱空、排水不畅或回填压实不足。

### AI 可直接引用

“RQI 薄弱更偏向行驶质量和平整度问题；若病害数量不多但 RQI 低，应重点复核沉陷、拥包、跳车和修补平整度。”""",
        "RDI 指标解释": """### 标准定位

RDI 表示车辙深度指数，用于识别车辙对路面使用性能和行车安全的影响。

### 直接判读规则

- RDI 低且车辙连续：需要结合车辙深度、连续长度、车道位置、重载比例和雨天积水情况判断风险。
- 车辙集中在重车道或物流出入口：优先考虑渠化交通和高温稳定性影响。
- 浅层流动变形可考虑铣刨加铺；结构性车辙或伴随沉陷时，应先复核基层和结构层，不得只做表面罩面。

### AI 可直接引用

“RDI 偏低说明车辙问题影响明显。若车辙深度较大、连续长度较长或雨天积水，应区分浅层变形和结构性车辙，再决定铣刨加铺、结构补强或排水改善。”""",
        "SCI/BCI/TCI 指标解释": """### 标准定位

SCI、BCI、TCI 分别用于反映路基、桥隧构造物、交通工程及沿线设施状况。它们会影响综合 MQI，但不能替代路面病害判断。

### 直接判读规则

- SCI 低：优先复核边坡、路肩、边沟、排水、防护和路基稳定。
- BCI 低：不得在路面养护建议中简单归因为裂缝坑槽，应提示桥隧专项检查或构造物养护。
- TCI 低：关注标志标线、护栏、排水设施、里程桩等沿线设施。
- MQI 被 SCI/BCI/TCI 拉低时，报告应明确“非路面分项影响”，避免把所有问题写成路面病害。

### AI 可直接引用

“综合指标被路基、桥隧或沿线设施拉低时，应单独说明分项问题，不应把所有养护建议都归入路面铣刨或坑槽修补。”""",
        "评定等级与判定规则": """### 标准等级口径

JTG 5210—2018 采用优、良、中、次、差五级表达技术状况。SRMP 默认按以下口径展示指标等级，项目如有地方细化口径，可在项目配置中覆盖：
...
- 等级输出必须同时给出指标值、等级、薄弱分项和主要证据。
- “良”不等于无需养护；若分项指标为中或病害集中，应提出针对性预防养护或复核建议。
- “次/差”不自动等于大修；应结合病害类型、范围、结构强度、排水和交通荷载确认处置层级。
- 农村公路可参考 JTG 5211—2024 的农村公路评定体系，并在项目配置中明确是否启用农村公路口径。

### AI 可直接引用

“当前等级应按指标值归入优、良、中、次、差，并解释薄弱分项。若 PCI 为中而 MQI 为良，建议写成‘总体尚可，但路面破损分项偏弱，需要针对裂缝、坑槽集中区处置’。”""",
        "指标异常组合判读卡": """### 用途

本卡用于处理“综合指标看起来还行，但某个分项明显异常”的场景，避免 AI 只按 MQI 总分给出泛泛结论。

### 常见组合与判读

| 指标组合 | 业务含义 | 优先追问/校验 | 建议处置方向 |
|---|---|---|---|
| MQI≥80 但 PCI<70 | 综合等级尚可，但路面破损已经成为薄弱项 | 裂缝、坑槽、松散、沉陷数量和分布；是否连续集中 | 对集中破损区做局部修补、灌缝、封层或局部铣刨重铺，不能只写“总体良好” |
| PCI<70 且 RQI≥85 | 路面破损明显，但行驶平顺性尚可 | 病害是否为早期裂缝、局部坑槽或表层松散 | 优先做破损处置和防水封闭，防止雨水下渗后扩大 |
| RQI<70 但 PCI≥80 | 破损不明显，但行驶质量差 | 平整度、沉陷、拥包、桥头跳车、连续修补不平整 | 做平整度和现场复核，必要时局部铣刨调平或桥涵过渡段处治 |
| RDI<70 且车辙连续长度超过 200m | 车辙已形成连续风险，雨天积水和侧滑风险增加 | 车辙深度、重载比例、渠化交通、雨天积水 | 区分浅层变形与结构性车辙，浅层可铣刨加铺，结构性需补强 |
| MQI<70 但各分项数据缺失 | 综合结果异常但证据不足 | 数据同步、图层范围、检测批次、字段映射 | 只能提示补充证据，不得直接输出大修或重铺结论 |

### AI 可直接给出的结论模板

- “当前 MQI 仍为良，但 PCI 已低于 70，说明综合状态掩盖了路面破损短板。建议先定位裂缝、坑槽、松散和沉陷集中区，再按病害类型确定灌缝、坑槽修补或局部铣刨重铺。”
- “当前 RQI 低而 PCI 不低，不能只按破损病害解释，应重点复核沉陷、拥包、桥头跳车和既有修补平整度。”
- “当前 RDI 低且车辙连续，应将雨天积水、重载渠化交通和结构性车辙作为现场复核重点。”

### 禁忌

- 不得只凭 MQI 等级判断“无需养护”。
- 不得把 PCI、RQI、RDI 的异常都解释成同一种病害。
- 不得在缺少病害明细、GIS 校验或现场复核时直接给出大修结论。""",
    }
    ...
    add("20_病害识别与分级", category_doc("DISEASE_STANDARD", "维护常见路面病害类型、识别标准、严重程度和复核口径。", [
        "裂缝类病害", "坑槽类病害", "车辙类病害", "沉陷/拥包/松散类病害", "病害严重程度分级", "裂缝-坑槽-沉陷关联判读卡",
    ]))
    disease_samples = {
        "裂缝类病害": """### 识别对象

裂缝类病害包括纵向裂缝、横向裂缝、网裂、块状裂缝、龟裂等。农村公路场景可参考 JTG 5211—2024 的简化思路，将龟裂与块状裂缝统一归入网裂管理。

### 现场判断要点

- 纵向裂缝：沿行车方向延伸，重点复核是否位于轮迹带、拼接缝、路肩边缘或拓宽接缝。
- 横向裂缝：横穿车道，重点复核温缩、基层反射裂缝、桥涵过渡段影响。
- 网裂/块状裂缝：关注裂缝网格、块度、密度和是否伴随松散、沉陷；网裂密集通常提示水损害或结构层问题风险。
- 追问字段：裂缝宽度、长度、密度、是否渗水、是否发展、是否伴随沉陷/松散。

### 处置建议

- 轻中度、基层稳定、未明显渗水：清缝、干燥后灌缝或封缝。
- 裂缝密集但结构稳定：可考虑封层、微表处或薄层罩面。
- 裂缝发展快、伴随沉陷/松散/唧浆：先现场复核基层和排水，再考虑局部铣刨重铺或结构补强。

### AI 可直接引用

“裂缝处置不能只看名称，应结合裂缝宽度、长度、密度、是否渗水和是否伴随沉陷。轻中度裂缝可灌缝/封缝，网裂密集或发展性裂缝需先复核基层和排水。”""",
        "坑槽类病害": """### 识别对象

坑槽是路面材料脱落形成的局部坑洞，常与裂缝渗水、松散、基层水损害、反复荷载和修补失效有关。

### 现场判断要点

- 安全风险优先：坑槽位于轮迹带、弯道、坡道、桥头或夜间照明不足路段时，应优先处置。
- 必查字段：坑槽深度、面积、边缘破碎、是否积水、是否露骨料/基层、周边裂缝和松散情况。
- 多坑槽连续出现：不宜逐点冷补了事，应复核是否存在排水或基层问题。

### 处置建议

- 存在即时安全风险：先设警示并做临时修补，保障通行。
- 永久修补：切边成规则形状、清底、干燥、喷洒粘层油、分层填补、压实、封边，开放交通前检查平整度和压实效果。
- 反复坑槽：复核基层松散、含水、脱空和排水系统，不得只做表面填补。

### AI 可直接引用

“坑槽处置应安全风险优先。深度较大、边缘破碎或位于行车轮迹带时，先临时修补保通，再按切边、清底、粘层、填补、压实流程做永久修补。”""",
        "车辙类病害": """### 识别对象

车辙是轮迹带沿行车方向形成的纵向凹槽，常见于重载、渠化交通、高温稳定性不足、面层流动变形或结构层压密变形。

### 现场判断要点

- 必查字段：车辙深度、连续长度、车道位置、重载车辆比例、雨天积水、是否伴随裂缝或推移拥包。
- 重车道连续车辙：优先判断高温流动变形与结构性车辙。
- 雨天积水车辙：需要关注水膜、侧滑和行车安全，不应只按舒适性问题处理。

### 处置建议

- 浅层车辙且基层稳定：铣刨加铺或功能性罩面可作为候选方案。
- 结构性车辙、伴随沉陷或反复出现：复核基层、路基、排水和交通荷载，再考虑结构补强。
- 只做薄层罩面不能解决结构性车辙，容易短期复发。

### AI 可直接引用

“车辙分析应看车辙深度、连续长度和重载渠化交通。若雨天积水或伴随沉陷，应按结构性问题复核，不能只用表面罩面掩盖。”""",
        "沉陷/拥包/松散类病害": """### 识别对象

沉陷表现为局部下沉，拥包表现为局部隆起或推移，松散表现为集料脱落、表面粗糙和材料散失。三类病害都可能指向结构层、材料、排水或施工质量问题。

### 现场判断要点

- 沉陷：复核范围、深度、是否积水、是否靠近涵洞/管线/填挖交界。
- 拥包：复核是否位于制动区、交叉口、坡道、重载起步区；判断面层高温稳定性或层间粘结问题。
- 松散：复核材料老化、油石比、压实不足、雨水冲刷和既有修补质量。

### 处置建议

- 沉陷伴排水不畅：先查排水和基层稳定，再确定挖补深度。
- 拥包推移明显：局部铣刨、重铺或重新压实，必要时改善材料和层间粘结。
- 松散发展：轻微可表面处治，严重或露骨料/露基层时应局部挖补。

### AI 可直接引用

“沉陷、拥包和松散不能只做表层修补。若伴随积水、脱空、基层松散或反复破坏，应先复核结构层和排水条件，再确定挖补或重铺范围。”""",
        "病害严重程度分级": """### 分级原则

严重程度应优先采用系统字段、项目检测规程或地方配置。AI 不得自行创造与项目不一致的等级名称。

### 判断要素

- 裂缝：宽度、长度、密度、网裂块度、是否渗水、是否发展。
- 坑槽：深度、面积、边缘破碎、是否位于轮迹带、是否影响安全。
- 车辙：车辙深度、连续长度、雨天积水和车道位置。
- 沉陷/拥包/松散：面积、深度/隆起高度、是否伴随结构性迹象。

### 输出要求

- 有系统等级：直接引用并解释依据。
- 无系统等级但有测量字段：给出“建议复核为轻/中/重”的条件化判断。
- 字段不足：提示需要补充现场复核，不得把轻微病害直接写成重度。

### AI 可直接引用

“严重程度应跟随项目字段；缺少字段时，只能基于宽度、深度、面积、连续长度和安全风险给出复核建议，不能替代人工判级。”""",
        "裂缝-坑槽-沉陷关联判读卡": """### 用途

本卡用于同一桩号范围内出现多个病害时的关联判读。AI 命中本卡后，不允许只按单一病害给出处置，应先说明病害组合可能指向的风险。

### 关联组合

| 病害组合 | 可能风险 | 必须追问/复核 | 输出边界 |
|---|---|---|---|
| 裂缝 + 坑槽 | 裂缝渗水后材料松散，坑槽可能继续扩展 | 裂缝是否渗水、坑槽深度、坑槽边缘是否破碎、雨后是否积水 | 可建议先保通、修补坑槽，再封闭裂缝防水 |
| 裂缝 + 沉陷 | 可能存在基层松散、脱空、填挖交界沉降或排水问题 | 沉陷深度、范围、是否唧浆、是否靠近涵洞/管线/边沟 | 不得只灌缝结案，应提示基层和排水复核 |
| 坑槽 + 松散 | 面层材料散失或修补质量差，可能快速复发 | 松散面积、露骨料/露基层、既有修补痕迹、含水情况 | 不得只表面填补，应按切边清底和压实闭环 |
| 车辙 + 拥包 | 重载渠化、高温流动变形或层间粘结问题 | 车辙深度、连续长度、制动区/坡道位置、重载比例 | 不能只做薄层罩面，应先判断浅层或结构性问题 |
| 网裂 + 唧浆 | 水损害或基层问题风险较高 | 雨后出浆、基层含水、排水系统、连续范围 | 应建议现场钻芯/开挖或专项复核，不直接给普通封层 |

### AI 可直接引用

“同一区间同时出现裂缝、坑槽和沉陷时，应按组合病害分析。裂缝负责解释水进入结构层的通道，坑槽说明材料脱落和安全风险，沉陷提示可能存在基层或路基问题；处置上应先保通，再复核基层和排水，最后确定修补或重铺范围。”

### 禁忌

- 不允许只按单一病害给出处置。
- 不允许把“裂缝 + 沉陷 + 唧浆”写成普通灌缝。
- 不允许在未完成 GIS 桩号校验时，把相邻路段的病害组合写进当前对象。""",
    }
    ...
    add("30_养护处置工艺", category_doc("MAINTENANCE_METHOD", "维护病害处置、预防性养护和结构性修复的工艺说明。", [
        "裂缝处置工艺", "坑槽修补工艺", "车辙与沉陷处置", "铣刨重铺", "灌缝/封层/罩面", "预防性养护措施选择卡",
    ]))
    method_samples = {
        "裂缝处置工艺": """### 适用条件

适用于轻中度裂缝、基层基本稳定、裂缝尚未形成明显沉陷或松散的场景。雨天、潮湿裂缝、低温不满足材料施工要求时不得施工。

### 工艺流程

1. 交通组织与安全布控。
2. 清缝：清除缝内尘土、杂物、松散料，必要时开槽。
3. 干燥：缝壁潮湿时需烘干，雨后未干不得直接灌缝。
4. 灌缝材料施工：按材料要求加热或搅拌，连续填充，避免空洞。
5. 表面处理：刮平、撒布防粘材料或贴缝带，防止车轮带出。
6. 开放交通：满足材料冷却、固化和粘结要求后开放交通。
7. 复查：雨季后检查是否脱落、开裂、渗水或继续发展。

### 禁忌

- 裂缝伴随沉陷、唧浆、松散或基层脱空时，不得只灌缝结案。
- 雨天不得施工；缝内潮湿未处理不得灌缝。

### AI 可直接引用

“裂缝灌缝应按清缝、干燥、灌缝材料填充、表面处理、开放交通和复查闭环执行；若裂缝伴随沉陷或松散，应先复核基层。”""",
        "坑槽修补工艺": """### 适用条件

适用于局部坑槽、边缘破碎、表层材料脱落等需要恢复通行安全和路面完整性的场景。安全风险高的坑槽应先临时修补，再安排永久修补。

### 永久修补流程

1. 交通安全布控，必要时设置限速和导改。
2. 切边：按规则形状切除松散边缘，边线尽量顺直。
3. 清底：清除松散料、泥水和杂物，检查是否露基层或有积水。
4. 干燥：坑槽潮湿时先排水、烘干或采取适配材料。
5. 粘层油：坑壁和坑底均匀喷洒，保证新旧料结合。
6. 分层填补：按厚度分层摊铺热料或合格修补料。
7. 压实：由边到中压实，保证平整度和密实度。
8. 封边与开放交通：检查接缝、平整度和压实后再开放。

### 禁忌

- 坑槽不能只做表面填料，未切边、未清底、未压实的修补易复发。
- 反复坑槽应排查基层松散、含水和排水问题。

### AI 可直接引用

“永久坑槽修补必须包含切边、清底、粘层油、分层填补和压实；若只是临时冷补，应明确后续永久修补时间和复核项。”""",
        "车辙与沉陷处置": """### 适用条件

适用于 RDI 偏低、车辙连续、局部沉陷、拥包推移或重载通道变形问题。核心是先区分功能性表层问题与结构性问题。

### 处置路径

- 浅层车辙：面层流动变形、基层稳定时，可采用局部铣刨、调平和加铺。
- 结构性车辙：伴随沉陷、裂缝、唧浆或反复变形时，应复核基层、路基和排水，必要时结构补强。
- 沉陷：确定沉陷范围和深度，检查涵洞、管线、填挖交界和排水，再确定挖补深度。
- 拥包：局部铣刨重铺，复核材料稳定性、层间粘结和重载制动影响。

### AI 可直接引用

“车辙和沉陷处置应先判断浅层变形还是结构性问题。浅层可铣刨加铺；结构性车辙或沉陷需基层复核、排水改善和结构补强。”""",
        "铣刨重铺": """### 适用条件

适用于连续病害、面层老化、反复坑槽、网裂密集、局部结构修复或需恢复平整度和抗滑性能的路段。零散轻微裂缝不应直接升级为铣刨重铺。

### 实施要点

1. 明确范围：路线、起止桩号、车道、宽度和病害分布。
2. 明确深度：区分功能性铣刨、面层重铺和结构性挖补。
3. 施工前复核：基层稳定、排水、既有修补、隐性病害和交通荷载。
4. 铣刨清扫：清除浮料和污染，保持下承层洁净。
5. 粘层与摊铺：保证层间结合，按设计材料和厚度施工。
6. 压实与检测：关注压实度、平整度、接缝、厚度和开放交通温度。
7. 后评估：复查 PCI/RQI/RDI 和病害复发情况。

### AI 可直接引用

“铣刨重铺适用于连续病害或面层老化区。输出方案时必须写明桩号范围、铣刨厚度、材料、交通组织、质量控制和后评估指标。”""",
        "灌缝/封层/罩面": """### 适用条件

灌缝、封层、微表处和薄层罩面属于常见预防性养护或功能性恢复手段。适用前提是结构强度基本满足、基层稳定、病害仍处早期或中轻度阶段。

### 技术选择

- 灌缝/封缝：适用于裂缝宽度可处理、尚未形成明显松散和沉陷的场景。
- 封层：用于封水、防止松散、延缓老化、改善表面功能；不解决结构性破坏。
- 微表处：适用于轻微车辙、表面磨耗、抗滑不足和早期老化，施工前需处理局部病害。
- 薄层罩面：可改善平整度、抗滑和表面老化；若基层或结构层已损坏，应先修复结构。

### 禁忌

- 预防性养护不得用于结构性破坏、深层水损害、严重沉陷或反复坑槽的替代处置。
- 封层不是大修替代品；只能延缓早期病害发展，不能恢复失稳结构。

### AI 可直接引用

“预防性养护适合结构强度基本满足且病害处于早期的路段。封层、微表处、薄层罩面可封水、延缓老化、改善表面功能，但不得替代结构性修复。”""",
        "预防性养护措施选择卡": """### 用途

本卡用于在路面结构基本稳定、病害处于早期或中轻度阶段时，选择雾封层、碎石封层、稀浆封层、微表处或薄层罩面等措施。结构性破坏禁用本卡直接决策。

### 选择表

| 措施 | 适用场景 | 前置处理 | 不适用场景 |
|---|---|---|---|
| 雾封层 | 表面轻微老化、细小裂隙、轻微松散，需要封水和恢复沥青膜 | 清扫、局部裂缝封闭、保证路面干燥 | 坑槽、沉陷、结构性裂缝、抗滑不足严重 |
| 碎石封层 | 需要增强表面封水、改善抗滑、延缓老化的低中交通量路段 | 修补坑槽、处理宽裂缝、清理浮尘 | 车辙严重、平整度差、基层不稳定 |
| 稀浆封层 | 表面磨耗、轻微松散、细小裂缝较多但结构稳定 | 先处置坑槽和明显裂缝 | 深层水损害、连续沉陷、反复坑槽 |
| 微表处 | 早期车辙、表面磨耗、抗滑不足、轻中度老化 | 局部病害修补，检查排水和粘结条件 | 结构性车辙、深层沉陷、基层松散 |
| 薄层罩面 | 需要改善平整度、抗滑和表面功能，原结构强度基本满足 | 处理裂缝、坑槽、松散和局部沉陷 | 严重网裂、基层失稳、反复坑槽未查明原因 |

### 决策顺序

1. 先看结构是否稳定：若存在沉陷、唧浆、反复坑槽或严重网裂，不进入预防性养护直接选择。
2. 再看功能目标：封水优先考虑雾封层/封层，抗滑和平整度优先考虑微表处/薄层罩面。
3. 最后看局部病害：所有坑槽、宽裂缝、松散和局部沉陷应先处置，再实施整体预防养护。

### AI 可直接引用

“预防性养护不是修复结构性破坏的替代品。若当前路段结构稳定且病害早期，可按目标选择雾封层、碎石封层、稀浆封层、微表处或薄层罩面；若存在沉陷、唧浆或反复坑槽，应先复核结构和排水。”

### 禁忌

- 结构性破坏禁用预防性养护直接覆盖。
- 不得用薄层罩面掩盖未查明原因的连续车辙或沉陷。
- 不得在雨天、潮湿基面或局部坑槽未修补时实施封层类措施。""",
    }
...
{{evidenceSummary}}"""), "40_方案模板蓝本")
    ...
    add("50_项目资料", category_doc("PROJECT_CONTEXT", "维护项目背景、区域策略、历史检测和数据口径。", [
        "项目背景资料", "区域养护策略", "历史检测资料", "数据口径说明", "山区农村公路风险因子库",
    ]))
    project_samples = {
        "项目背景资料": """### 适用场景
...
本卡用于山区、乡镇、农村公路项目的养护优先级加权。它不是病害事实来源，只能作为风险解释和排序辅助，必须与当前 GIS 对象、评定指标和病害台账一起使用。

### 风险因子

| 风险因子 | 现场含义 | 对养护排序的影响 |
|---|---|---|
| 临水临崖 | 路侧安全冗余低，边坡、挡墙、防护设施和排水风险更敏感 | 中重度病害、沉陷、边沟淤塞应提高优先级 |
| 急弯陡坡 | 制动、转向和视距风险增加，坑槽、车辙、抗滑不足更危险 | 轮迹带坑槽、车辙和松散优先处置 |
| 学校、医院、赶集点 | 人流集中、慢行交通多，通行安全敏感 | 病害严重程度相同时，优先保障通学、就医和集市通道 |
| 雨季排水 | 暴雨后裂缝渗水、坑槽扩展、边沟冲刷和路基软化风险上升 | 裂缝封水、边沟清淤、涵洞疏通前置 |
| 重载绕行或物流通道 | 车辙、拥包和结构层疲劳风险更高 | RDI 低、连续车辙和反复修补区优先复核 |

### 优先级加权口径

1. 先按技术状况排序：PCI/RQI/RDI、病害严重程度和连续长度。
2. 再按道路功能加权：通学、就医、客运、物流和应急通道。
3. 最后按环境风险加权：临水临崖、急弯陡坡、雨季排水和边坡防护。

### AI 可直接引用

“该区间若同时具备临水临崖、急弯陡坡、学校/医院通道和雨季排水压力，应在同等 PCI 或病害严重程度下提高优先级；但优先级加权不能替代 GIS 校验和病害事实。”

### 禁忌

- 不得把风险因子写成已经发生的病害。
- 不得在没有地图对象或区域范围时，笼统判断整条路线都是高风险。
- 不得只按背景加权，忽略当前评定和病害数据。""",
    }
    ...
    add("60_案例库", category_doc("CASE_LIBRARY", "沉淀典型养护方案、处置案例和问题复盘。", [
        "优秀方案案例", "典型病害处置案例", "问题复盘", "连续低分路段处置案例",
    ]))
    case_samples = {
        "优秀方案案例": """### 案例场景
...
某山区农村公路 K2+000—K2+800 连续 800m 路段，上一轮 PCI 为 78，本轮 PCI 从 78 降至 62；病害台账显示横向裂缝 18 条、纵向裂缝 9 条、坑槽 6 处、局部沉陷 2 处，雨后边沟淤塞明显。

### GIS 与证据校验

- 地图定位：病害点均落在 K2+000—K2+800 范围内，未把相邻 K1+900 或 K2+900 的点误并入。
- 指标证据：PCI 降至 62，RQI 为 76，RDI 为 84，说明主要矛盾是路面破损，不是车辙。
- 病害证据：裂缝与坑槽在 K2+300—K2+650 更集中，局部沉陷靠近边沟淤塞段。
- 来源上下文：指标来自评定结果，病害来自地图图层，处置依据来自裂缝处置工艺、坑槽修补工艺和预防性养护措施选择卡。

### 处置组合

1. 先进行排水边沟清淤，处理 K2+300—K2+650 段积水风险。
2. 对轻中度裂缝实施裂缝灌缝，要求清缝、干燥、灌缝材料填充、表面处理和复查闭环。
3. 对坑槽执行永久修补：切边、清底、粘层油、分层填补、压实。
4. 对 K2+420—K2+520 反复破损和沉陷段，现场复核基层后实施局部铣刨重铺。
5. 完成后复评 PCI 和坑槽复发情况，雨季后复查边沟和裂缝封闭效果。

### AI 可直接引用

“类似 K2+000—K2+800 这种 PCI 连续下降、裂缝和坑槽集中、且排水边沟淤塞的路段，应采用排水先行、裂缝灌缝、坑槽永久修补、局部铣刨重铺和复评跟踪的组合措施。”

### 禁忌

- 不得只因 PCI 为 62 就整段大修。
- 不得只做裂缝灌缝而忽略坑槽和排水。
- 不得把 K2+000—K2+800 的案例工程量直接套给当前路线，应先做 GIS 校验和现场复核。""",
    }
    ...
    add("70_术语与问答", category_doc("FAQ", "维护指标、工艺和平台使用相关常见问答。", [
        "指标 FAQ", "养护工艺 FAQ", "平台使用 FAQ", "AI 追问来源上下文 FAQ",
    ]))
    faq_samples = {
        "指标 FAQ": """### Q1：PCI 和 PQI 有什么区别？

PCI 是路面损坏状况指数，重点解释裂缝、坑槽、松散、沉陷等破损。PQI 是路面使用性能综合指标，需要结合 PCI、RQI、RDI 等分项判断。回答时应先说明差异，再结合当前对象数据：若 PCI 更弱，应追问病害明细；若 RQI/RDI 更弱，应追问平整度或车辙数据。

### Q2：MQI 是良，为什么还要养护？

MQI 是综合结果，“良”不代表所有分项都好。若 PCI 为中、裂缝坑槽集中，仍应针对薄弱区做预防养护或局部处置。

### Q3：指标为次/差是否一定大修？

不一定。次/差说明技术状况较弱，但工法选择还要看病害类型、连续范围、结构强度、排水和交通荷载。AI 不得只凭等级输出大修结论。""",
        "养护工艺 FAQ": """### Q1：裂缝一定要铣刨重铺吗？

裂缝不一定需要铣刨重铺。轻中度裂缝、基层稳定、未明显渗水时，可优先清缝、干燥、灌缝或封缝；裂缝密集但结构稳定时，可考虑封层、微表处或薄层罩面；伴随沉陷、松散、唧浆或发展较快时，才需要复核基层并考虑局部铣刨重铺或结构补强。

### Q2：坑槽能不能直接填上？

坑槽不能只做表面填料。永久修补应包含切边、清底、粘层油、分层填补和压实；安全风险高时可先临时修补保通，但必须安排永久修补和基层/排水复核。

### Q3：封层能不能代替大修？

封层不是大修替代品。封层主要用于封水、延缓老化、改善表面功能；如果存在结构性破坏、严重沉陷、反复坑槽或基层松散，应先处理结构问题。""",
        "平台使用 FAQ": """### Q1：为什么生成方案前要先分析对象？

分析会收集业务证据和知识依据：地图对象、路线桩号、评定指标、病害明细、GIS 校验结果和 Outline 知识命中。没有这些证据，方案容易变成套话。

### Q2：用户追问“来源是什么”时怎么答？

应返回来源上下文：命中的知识标题、Outline 文档来源、地图对象类型、路线桩号、指标字段、病害字段、GIS 校验状态和仍需补充的数据。不能只回答“来自知识库”。

### Q3：点击定位和 GIS 校验有什么关系？

点击定位用于把来源证据定位到地图对象；GIS 校验用于确认对象是否落在当前路线、桩号或区域范围内。AI 回答应优先使用校验通过的对象，未通过时要提示可能存在空间范围不一致。""",
        "AI 追问来源上下文 FAQ": """### Q1：用户点击来源后，AI 应该怎么处理？

点击来源后应先定位到地图对象。如果来源是病害、评定单元、路线或区域统计，系统应读取来源中的对象 ID、路线编码、桩号、图层类型和年份，再触发 GIS 校验，确认该对象确实位于当前地图上下文内。

### Q2：什么是 GIS 校验？

GIS 校验是把 AI 证据来源和地图空间对象重新对齐：校验路线编码是否一致、桩号是否落在范围内、几何是否落在当前区域、图层是否开启、对象是否仍存在。校验通过后才能把来源写成“当前地图对象的证据”。

### Q3：用户追问来源上下文时，应该返回哪些内容？

追问来源上下文时，应返回四类信息：

- 知识来源：Outline 文档标题、知识类型、命中的段落或关键词；
- 业务来源：地图对象类型、对象 ID、路线、桩号、指标字段或病害字段；
- GIS 状态：是否通过 GIS 校验、是否已完成地图定位、是否存在范围不一致；
- 输出边界：哪些结论来自知识依据，哪些结论来自当前业务数据，哪些仍需现场复核。

### Q4：知识来源和空间事实有什么区别？

知识来源只能解释判定口径和处置依据，不等于当前路段已经发生该病害。比如命中了“坑槽修补工艺”，只能说明坑槽该怎么修；是否存在坑槽，必须来自当前病害图层、台账或 GIS 校验结果。不要把知识来源当成空间事实。

### Q5：AI 可直接引用的回答模板是什么？

“这条建议引用了 Outline 知识《坑槽修补工艺》，用于说明永久修补应包含切边、清底、粘层油、分层填补和压实；当前空间事实来自病害图层中的对象 `{{objectId}}`，位置为 `{{routeCode}} {{stakeRange}}`，GIS 校验结果为通过。若需要确定基层是否损坏，还需要现场复核。”

### 禁忌

- 不要只回答“来自知识库”，必须说明具体知识标题和业务对象。
- 不要把知识来源当成空间事实。
- 不要在 GIS 校验失败时继续声称“当前地图对象存在该病害”。
- 来源上下文只解释知识依据和业务证据边界，不能替代现场检测结论。""",
    }
