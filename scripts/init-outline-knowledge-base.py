...
{{evidenceSummary}}"""), "40_方案模板蓝本")
    ...
    project_samples = {
        "项目背景资料": "样例：项目位于山区农村公路，部分路线重载车辆较多，雨季排水问题突出。AI 生成报告时可引用项目背景解释病害集中原因。",
        "区域养护策略": "样例：优先保障学校、医院、物流通道周边道路通行安全；连续病害区优先采用集中处置。",
        "历史检测资料": "样例：上一轮检测显示 PCI 持续下降的区间，本轮若病害数量继续上升，应提高处置优先级。",
        "数据口径说明": "样例：病害面积、长度、严重程度以项目导入台账为准；评定指标以当前项目最新评定结果为准。",
    }
    ...
    case_samples = {
        "优秀方案案例": "样例：某路线低分区间采用裂缝灌缝、局部铣刨重铺和排水改善组合措施，实施后次年 PCI 明显提升。",
        "典型病害处置案例": "样例：连续坑槽区先进行临时冷补保障安全，再安排热拌料永久修补和基层排查。",
        "问题复盘": "样例：某次方案只按病害数量排序，忽略学校周边通行风险，后续应把业务敏感区域纳入优先级。",
    }
