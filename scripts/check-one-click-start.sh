...
test -f scripts/srmp-one-click-start-dev.sh
test -f scripts/srmp-init-demo.sh
test -f scripts/srmp-check-dev-schema.sh
...
bash -n scripts/srmp-one-click-start-dev.sh
bash -n scripts/srmp-init-demo.sh
bash -n scripts/srmp-check-dev-schema.sh
...
grep -R "docker exec -i srmp-postgres env PGPASSWORD" -n scripts/srmp-init-demo.sh >/dev/null
...
grep -R "srmp-check-dev-schema.sh" -n scripts/srmp-one-click-start-dev.sh >/dev/null
grep -R "DEV_SCHEMA_PREFLIGHT" -n scripts/srmp-one-click-start-dev.sh >/dev/null
grep -R "data_mgmt_project.archived" -n scripts/srmp-check-dev-schema.sh >/dev/null
grep -R 'Frontend:  http://localhost:${FRONTEND_PORT:-5173}' -n scripts/srmp-one-click-start-dev.sh >/dev/null
