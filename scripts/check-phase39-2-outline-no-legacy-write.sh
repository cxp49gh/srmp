...
echo "==> 检查 Outline 同步是否仍写 legacy knowledge_document / knowledge_chunk"
...
grep -R "aiKnowledgeIngestService.ingestMarkdown" srmp-agent/src/main/java/com/smartroad/srmp/agent/outline/service/impl/OutlineSyncServiceImpl.java \
  || { echo "[FAIL] Outline 同步未走 AiKnowledgeIngestService"; exit 1; }
