<template>
  <div class="demo-page">
    <header class="header">
      <div>
        <h1>演示数据联调看板</h1>
        <p>检查演示数据是否真正接入 GIS、AI 问答和方案生成。</p>
      </div>
      <div class="actions">
        <el-select v-model="tenantId" style="width: 180px" placeholder="tenantId">
          <el-option label="default" value="default" />
          <el-option label="demo" value="demo" />
          <el-option v-for="t in availableTenants" :key="t.tenant_id" :label="`${t.tenant_id} (${t.data_count})`" :value="t.tenant_id" />
        </el-select>
    ...
    <el-alert v-if="error" class="mb" type="error" show-icon :closable="false" :title="error" />
    ...
    <el-alert
      ...
      show-icon
      :closable="false"
      :title="status.health.ready ? `演示数据已就绪：${status.tenantId}` : `演示数据未完全就绪：${status.tenantId}`"
    >
      <template #default>
        请求租户：{{ status.requestedTenantId }}；实际读取租户：{{ status.tenantId }}。
        计数：{{ status.tables }}
      </template>
    </el-alert>
    ...
    <el-empty v-if="!loading && !error && isEmptyDashboard" description="暂无演示数据。请先导入 srmp-demo-road-data-phase1-22-fixed.zip，或切换 tenantId 后刷新。" />
    ...
    <template v-else>
      <section class="main-grid">
        <el-card>
          ...
          <el-table :data="dashboard?.routeRanking || []" height="420" size="small">
            ...
            <el-table-column prop="length_km" label="里程" width="80" />
        ...
        <el-card>
          <template #header>等级 / 病害分布</template>
          <div class="tag-list">
      ...
      <section class="main-grid">
        <el-card>
          ...
          <el-table :data="dashboard?.lowScoreUnits || []" height="320" size="small">
            <el-table-column prop="route_code" label="路线" width="80" />
        ...
        <el-card>
          <template #header>AI 快捷问题</template>
...
const error = ref('')
const availableTenants = ref<Record<string, any>[]>([])
...
const statCards = computed(() => {
  ...
  return [
    ...
    { label: '路段数', value: s.section_count ?? '-' },
...
const isEmptyDashboard = computed(() => {
  const s = dashboard.value?.summary || {}
  return Number(s.route_count || 0) === 0 && Number(s.assessment_count || 0) === 0 && Number(s.disease_count || 0) === 0
})
...
async function load() {
  ...
  try {
    ...
    availableTenants.value = dashboard.value?.availableTenants || status.value?.availableTenants || []
    if (dashboard.value?.tenantId && dashboard.value.tenantId !== tenantId.value) {
      ElMessage.warning(`当前 ${tenantId.value} 无数据，已自动读取 ${dashboard.value.tenantId}`)
    }
  } catch (e: any) {
    error.value = e?.message || '加载演示数据失败，请查看浏览器 Network 和后端日志'
  } finally {
    loading.value = false
  }
}
...
function gradeLabel(grade: string) {
  const map: Record<string, string> = { EXCELLENT: '优', GOOD: '良', MEDIUM: '中', POOR: '次', BAD: '差' }
  return map[grade] || grade
}
</script>

<style scoped>
.demo-page { min-height: 100vh; background: #f1f5f9; color: #0f172a; padding: 20px; }
...
.stat-grid { display: grid; grid-template-columns: repeat(4, minmax(160px, 1fr)); gap: 12px; margin-bottom: 16px; }
.stat-value { font-size: 26px; font-weight: 800; }
.stat-label { color: #64748b; margin-top: 6px; }
.main-grid { display: grid; grid-template-columns: minmax(520px, 1.3fr) minmax(360px, 0.7fr); gap: 16px; margin-bottom: 16px; }
.tag-list { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 18px; }
.question-list { display: flex; flex-direction: column; gap: 10px; }
.question { display: flex; justify-content: space-between; align-items: center; gap: 12px; background: #f8fafc; border-radius: 10px; padding: 12px; }
</style>