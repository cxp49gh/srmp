...
async function send(){ const text=input.value.trim(); if(!text) return; messages.value.push({role:'user',content:text}); input.value=''; loading.value=true; try{ const r=await chat({message:text,context:props.context,mapObject:props.mapObject}); messages.value.push({role:'assistant',content:r?.answer || JSON.stringify(r)}) }catch(e:any){messages.value.push({role:'assistant',content:e?.message||'请求失败'})} finally{loading.value=false} }
</script>
<style scoped>
