<template>
  <AgentPageShell title="Outline 同步入库" description="将 Outline 文档同步到本地 AI 向量知识库，并完成向量化闭环验证。">
    <div class="stats-grid">
      <el-card shadow="never"><div class="stat-value">{{ value(stats, 'documentCount') || 0 }}</div><div class="stat-label">Outline 文档</div></el-card>
      <el-card shadow="never"><div class="stat-value">{{ value(stats, 'chunkCount') || 0 }}</div><div class="stat-label">Chunk 总数</div></el-card>
      <el-card shadow="never"><div class="stat-value success">{{ value(stats, 'embeddedChunkCount') || 0 }}</div><div class="stat-label">已向量化</div></el-card>
    ...
    <div class="sync-page">
      <el-card class="left-card">
        <template #header><div class="card-header"><span>同步配置</span></div></template>
        ...
        <el-alert type="info" show-icon :closable="false" class="mb" title="推荐流程：先 Dry Run → 正式同步 → 查看待补向量 → 点击补向量 → 再验证知识库检索。" />
      </el-card>
      ...
      <el-card class="right-card">
        ...
        <div v-for="item in tasks" :key="item.id" class="task-item" @click="openTask(item)">
          <div class="task-title"><strong>{{ value(item, 'status') }}</strong><div><el-tag v-if="truthy(value(item, 'dryRun', 'dry_run'))" size="small" type="info">DRY</el-tag><el-tag size="small" :type="tagType(value(item, 'status'))">{{ value(item, 'syncMode', 'sync_mode') }}</el-tag></div></div>
          <p>{{ value(item, 'id') }}</p>
          <div class="task-grid"><span>总数：{{ value(item, 'totalCount', 'total_count') || 0 }}</span><span>成功：{{ value(item, 'successCount', 'success_count') || 0 }}</span><span>跳过：{{ value(item, 'skipCount', 'skip_count') || 0 }}</span><span>失败：{{ value(item, 'failCount', 'fail_count') || 0 }}</span></div>
          <div v-if="value(item, 'errorMessage', 'error_message')" class="error">{{ value(item, 'errorMessage', 'error_message') }}</div>
          <div class="task-actions"><el-button size="small" @click.stop="openTask(item)">明细</el-button><el-button v-if="Number(value(item, 'failCount', 'fail_count') || 0) > 0" size="small" type="warning" :loading="retrying" @click.stop="retryFailed(item)">重试失败</el-button></div>
        </div>
      </el-card>
    </div>
    ...
    <el-drawer v-model="detailVisible" title="Outline 同步明细" size="82%">
      <template v-if="selectedTask">
        ...
        <el-table :data="details" border height="calc(100vh - 300px)" size="small" row-key="id">
          ...
          <el-table-column label="原因/错误" min-width="320" show-overflow-tooltip><template #default="{ row }"><span v-if="value(row, 'status') === 'FAILED'" class="error">{{ value(row, 'errorType', 'error_type') }}：{{ value(row, 'errorMessage', 'error_message') }}</span><span v-else>{{ value(row, 'detailMessage', 'detail_message') || value(row, 'skipReason', 'skip_reason') || '-' }}</span></template></el-table-column>
        </el-table>
      </template>
    </el-drawer>
  </AgentPageShell>
...
.doc-item p, .task-item p { margin: 4px 0; color: #64748b; word-break: break-all; }
.task-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; margin-top: 8px; }
...
.summary-item { padding: 8px 12px; border-radius: 10px; background: #f1f5f9; font-size: 13px; }
.summary-item.success { background: #ecfdf5; color: #047857; }
