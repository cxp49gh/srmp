...
### 方案 B：新增统一 `/run`，旧入口短期适配到 `/run`

新增 `POST /api/agent/map-agent/run`，请求中明确 `action` 和 `mapContext`。旧 `chat`、对象方案、区域方案入口只作为短期适配器调用 `run`。优点是边界清晰，前端可逐步迁移，风险可控。

### 方案 C：一次性删除旧入口，前端全量重写

最终状态最干净，但风险最大，容易同时影响一张图、方案预览、草稿保存和 Ops 调试。

推荐采用 **方案 B**：以 `/run` 作为新主干，旧入口只保留一个过渡版本的 adapter，不再保留 native 行为。
...
- `/chat` 调用 `/run` adapter，`action=CHAT`。
- `/api/agent/map-object/solution` 调用 `/run` adapter，`action=GENERATE_OBJECT_SOLUTION`。
- `/api/gis/map-region/solution` 调用 `/run` adapter，`action=GENERATE_REGION_SOLUTION`。

旧接口只作为前端迁移期兼容，不保留 native 行为。
...
保留 `MapAiAgentRequest/Response` 作为旧 adapter 的兼容类型，但新主干使用 Run schema。
...
2. **前端与工作流 PR**
   ...
   - solution 工具升级。
   - 删除 native 类和 parity 脚本。

这样可以先让新主干跑起来，再做组件拆分和文件删除。
